...
collide (Ball x1 y1 dx dy) (Wall x2 y2 w h) = Ball x1 y1 (if x1 < x2+w && x1+actorW > x2 then -5 else dx) (if y1 < y2+h && y1+actorH > y2 then -5 else dy) 
...
instance Actor BasicActor where
    ...
    move _ _ w@(Wall _ _ _ _) = [w]

----

main = do
    rnd <- newStdGen
    operateArena (Arena [Ball 200 100 5 5, Ball 230 120 (-5) (-5), Ghost 100 100 rnd, Turtle 160 120 False, Wall 50 50 100 20])
